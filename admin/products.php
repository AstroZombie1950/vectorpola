...
// Акционные товары (для отдельной вкладки)
$promoData = productsListPaged('', '', 1, 200, false, true);
...
<div class="tabs">
	...
	<button class="tab-btn <?= $defaultTab === 'promo' ? 'tab-btn--active' : '' ?>" data-tab="tab-promo">Акционные <span class="tab-count"><?= $promoData['total'] ?></span></button>
...
<!-- ============ АКЦИОННЫЕ ============ -->
<div class="tab-pane <?= $defaultTab === 'promo' ? 'tab-pane--active' : '' ?>" id="tab-promo">
	<p class="text-muted" style="margin-bottom:14px">Товары с галкой «Акционный» — показываются в блоке «Ежедневные акционные товары» на главной. Чтобы убрать из блока — откройте товар и снимите галку.</p>

	<?php if (empty($promoData['items'])): ?>
		<div class="empty-state">
			<p>Пока нет акционных товаров. Откройте любой товар на редактирование и включите «Акционный товар (на главной)».</p>
		</div>
	<?php else: ?>
		<div class="products-table-wrap">
			<table class="products-table">
				...
				<tbody>
				<?php foreach ($promoData['items'] as $p): ?>
					<tr data-id="<?= htmlspecialchars($p['id']) ?>">
						<td class="td-img">
							<?php if (!empty($p['images'][0])): ?>
								<img src="<?= htmlspecialchars($p['images'][0]) ?>" alt="">
		...
		<div class="form-cols">
			...
			<div class="form-col">
				<div class="card">
					...
					<label class="toggle-label" style="margin-top:10px">
						<input type="checkbox" name="promo" id="promoCheck" <?= !empty($editProduct['promo']) ? 'checked' : '' ?>>
						<span class="toggle"></span>
						Акционный товар (на главной)
					</label>
				</div>
