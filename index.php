...
$promo   = vp_promo_products(8);     // помеченные галкой «Акционный» в админке
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<?php include $_SERVER['DOCUMENT_ROOT'] . '/source/include/header.php'; ?>
	...
	<?php if ($promo): ?>
	<!-- ===== Акционные товары (наполняется галкой «Акционный» в админке) ===== -->
	<section class="section section--soft" id="promo">
		<div class="container">
			<!-- Интро-карточка (текст из ТЗ / референса) -->
			<div class="promo-intro">
				<div class="promo-intro__text">
					<h2>Акционные товары</h2>
					<p>Мы предлагаем ряд товаров по специальной акционной цене, что позволит вам существенно сэкономить на ремонте.</p>
				</div>
				<div class="promo-intro__icon" aria-hidden="true">
					<!-- Пакет со «%» в стиле сайта (stroke, акцент) -->
					<svg width="72" height="72" viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M12 16h24l-2 24a2 2 0 0 1-2 1.8H16a2 2 0 0 1-2-1.8L12 16z"/>
						<path d="M18 16v-2a6 6 0 0 1 12 0v2"/>
						<circle cx="20.5" cy="25.5" r="1.8"/>
						<circle cx="27.5" cy="32.5" r="1.8"/>
						<line x1="29" y1="23" x2="19" y2="35"/>
					</svg>
				</div>
			</div>

			<!-- Карусель акционных (стрелки — тот же механизм, что в «Популярных») -->
			...
			<div class="products">
				<?php foreach ($promo as $p):
					$pImg   = !empty($p['images'][0]) ? $p['images'][0] : '/source/img/no-image.webp';
					$pPrice = (float)($p['price'] ?? 0);
					$pOld   = (isset($p['old_price']) && $p['old_price']) ? (float)$p['old_price'] : null;
				?>
				<a class="product" href="<?= htmlspecialchars(vp_product_url($p)) ?>">
					<div class="img"><img src="<?= htmlspecialchars($pImg) ?>" alt="<?= htmlspecialchars($p['name']) ?>" width="800" height="800" loading="lazy" onerror="this.onerror=null;this.src='/source/img/no-image.webp'"></div>
					<div class="body">
						<div class="name"><?= htmlspecialchars($p['name']) ?></div>
						<div class="price">
							<?= vp_money($pPrice) ?> <small>/ <?= htmlspecialchars($p['unit'] ?? 'м²') ?></small>
							<?php if ($pOld && $pOld > $pPrice): ?><span class="price-old"><?= vp_money($pOld) ?></span><?php endif; ?>
						</div>
					</div>
				</a>
				<?php endforeach; ?>
		...
		<div class="container">
			<div class="subscribe">
				<div class="subscribe__text">
					<h2>Хотите первым знать о новых акциях?</h2>
					<p class="subscribe__lead">Заполните форму и получайте уведомления о запуске новых акций на E-mail</p>
					<p class="subscribe__consent">Отправляя заявку, вы соглашаетесь с правилами обработки персональных данных и политикой конфиденциальности</p>
				</div>
				<form class="subscribe-form" data-endpoint="/source/php/subscribe.php" data-success="Готово! Вы подписаны на уведомления об акциях." autocomplete="off">
					<div>
					...
					<button type="button" class="btn btn--accent">Подписаться</button>
